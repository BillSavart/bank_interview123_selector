...
- 留言列表最多顯示約 5 則高度，超過後在留言區內部捲動，避免題目卡片被大量留言撐長。
...
若要在本機測試參考答案評分、留言與留言投票，另開一個 terminal 啟動輕量 API：

```bash
PORT=3000 \
RATINGS_FILE=/tmp/bank-ratings-local.json \
COMMENTS_FILE=/tmp/bank-comments-local.jsonl \
COMMENT_VOTES_FILE=/tmp/bank-comment-votes-local.jsonl \
COMMENT_HIDE_SCORE=-100 \
npm run api
```

Vite dev server 會把 `/api/*` proxy 到 `http://127.0.0.1:3000`。若只測靜態題庫與答案展開，不啟動 API 也能瀏覽，評分與留言會載入失敗但不影響主畫面。
